...
void NewMixPaper::on_pBPrivious_clicked()
{  saveAnswers();
...
void NewMixPaper::on_pBNext_clicked()
{    saveAnswers();
    ui->lPageNo->setText(QString::number(ui->lPageNo->text().toInt()+1));
...
void NewMixPaper::drowEditQuestions(QDomElement root)
{
    int qNoInPage=1;
...
void NewMixPaper::saveEditedQuestions()
{
    QFile newPaperFile(paperCrationPath);
    if(!newPaperFile.open(QFile::ReadWrite| QIODevice::Text))
    {
...
void NewMixPaper::on_pBEPrivious_clicked()
{
    saveEditedQuestions();
...
void NewMixPaper::on_pBENext_clicked()
{
    saveEditedQuestions();
     ui->lEPageNo->setText(QString::number(ui->lEPageNo->text().toInt()+1));
